...
# model_path = 'Tracking_DeepSORT/deep_sort/onnx_models/yolov5s.onnx'
# ort_sess = ort.InferenceSession(model_path, providers=['CUDAExecutionProvider'])
# print(ort.get_device())

# img = cv2.imread('Tracking_DeepSORT/traffic.jpg')
# img = cv2.resize(img, (640, 640))
# np_img = img.reshape(1, 3, 640, 640)
# np_img = np_img.astype(np.float32, copy=False)


# print(np_img.shape)

# outs = ort_sess.run(['output'], {"images": np_img})
...
'''
class YOLO_GPU(): # outputs tlbr bounding boxes
    '''
    ...
    def pre_process(self, input_image):
        ...
        '''
        self.image = cv2.resize(input_image, (640, 640))
        self.original_img = input_image
        np_img = self.image.reshape(1, 3, 640, 640)
        np_img = np_img.astype(np.float32, copy=False)
        self.outputs = self.ort_sess.run(['output'], {"images": np_img})
    ...
    def post_process(self):
        ...
        image_height, image_width = self.original_img.shape[:2]
    ...
    #       box = boxes[i]
            ...
            right = box[2]
            bottom = box[3]
            cv2.rectangle(self.original_img, (left, top), (right, bottom), self.BLUE, 3*self.THICKNESS)

            #Display the label at the top of the bounding box
            labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
            top = max(top, labelSize[1])
            cv2.putText(self.original_img, labeltoDraw, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), thickness=1)
...
if __name__ == "__main__":
    ...
    cv2.imshow('output', yolo_detector.original_img)
